#THIS CODE IS MY OWN CODE, IT WAS WRITTEN WITHOUT CONSULTING A TUTOR OR CODE WRITTEN BY OTHER STUDENTS - TIANYI LU
# bustersAgents.py
# ----------------
...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        goalCo=None
        goalProb=0
        for i, co in enumerate(maxBelief):
            if livingGhostPositionDistributions[i][co] >= goalProb:
                goalCo=co
                goalProb=livingGhostPositionDistributions[i][co]
        minDist=9999
        bestAct=None
        for action in legal:
            nextLoc = Actions.getSuccessor(pacmanPosition, action)
            if self.distancer.getDistance(nextLoc, goalCo)<minDist:
                minDist=self.distancer.getDistance(nextLoc, goalCo)
                bestAct=action
        return bestAct
